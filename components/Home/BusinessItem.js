...
function BusinessItem({business,showDir=false}) {
    ...
    const onDirectionClick = () => {
        window.open(`https://www.google.com/maps/dir/?api=1&origin=${userLocation.lat},${userLocation.lng}&destination=${selectedBusiness.geometry.location.lat},${selectedBusiness.geometry.location.lng}&travelmode=driving`);
    }
    ...
    return (
        ...
        bg-white hover:scale-110 transition-all mt-[20px] cursor-pointer'>
            <Image src={`https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${photo_ref}&key=${GOOGLE_MAP_API_KEY}`} alt={business.name} width={180} height={80}
         ...
         {showDir && <div className='border-t-[1px] p-1 mt-1'>
              ...
              border-blue-500
              hover:text-white
              hover:bg-blue-500' onClick={()=>onDirectionClick()}>Get Direction</div>
            </div>
            }
        </div>
    );
